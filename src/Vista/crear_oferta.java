...
public class crear_oferta extends javax.swing.JFrame {
    ...
    public void MensajeGuardar(String respuesta) {
        ...
        String mensaje = "";
        if (!respuesta.equals("<NewDataSet />")) {
            try {
                SAXBuilder builder = new SAXBuilder();

                InputStream stream = new ByteArrayInputStream(respuesta.getBytes("UTF-8"));
                Document document = builder.build(stream);
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    String r_codigo = node.getChildText("R_CODIGO");
                    mensaje = node.getChildText("R_MENSAJE");
                }
